...
- **Display.** Always dark — warm gray text on near-black, like a Kindle. CSS `filter: invert(0.88) hue-rotate(180deg)` applied permanently. No toggle, no options. Books do not have display settings.
...
- **CSS inversion inverts everything.** Including any embedded images. Acceptable for text-only documents.
...
**Display.** Always dark — warm gray text on a near-black background via CSS `filter: invert(0.88) hue-rotate(180deg)`. The inversion ratio of 0.88 produces warm gray rather than harsh white. There is no light mode, no toggle, no settings. Books do not have display options.
...
**Reader visits the site.** Browser loads index.html. The reader sees warm gray text on a dark background — the page is immediately readable, like a Kindle. Adjacent pages preload silently. The reader presses the right arrow or taps the right half of the screen; the next page appears instantly. A brief "2 / 7" indicator fades in and disappears. Citations appear as numbered references in the body — as with a book, the reader flips to the bibliography. They share the URL `jeremyjacobson.dev/yatp/#3` with a colleague, who opens it directly to page 3.
...
**5. Invisible infrastructure.** The HTML, JavaScript, and CSS in the viewer are invisible. The reader does not know they exist. There are no buttons, no sidebars, no browser-styled UI elements, no settings, no options. The page fills the screen. The page indicator appears briefly and vanishes. The system's infrastructure should be felt, not seen.
...
- Kindle-style display. Always dark, warm gray text on near-black. No settings, no options, no second build.
...
- **Second LaTeX build with inverted colors for true dark SVGs.** Would avoid CSS filter artifacts on embedded images. Not needed for text-only documents where CSS inversion works well.
