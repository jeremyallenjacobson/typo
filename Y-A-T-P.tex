...
% --- page navigation for SVG output ---
\newcommand{\svgname}{Y-A-T-P}
\pagestyle{fancy}
\fancyhf{}
...
\fancyfoot[C]{%
  \small
  \ifnum\value{page}>1
    \href{\svgname-\the\numexpr\value{page}-1\relax.svg}{$\langle$\,Prev}%
    \quad\quad
  \fi
  \thepage\,/\,\pageref*{LastPage}%
  \ifnum\value{page}<\getpagerefnumber{LastPage}\relax
    \quad\quad
    \href{\svgname-\the\numexpr\value{page}+1\relax.svg}{Next\,$\rangle$}%
  \fi
}
\fancypagestyle{plain}{\pagestyle{fancy}}

\title{Yet Another Theory of Programming}
\author{}
\date{}
...
those discussed here, I have chosen not to delay the writing of this
document by reviewing the literature beyond the references cited
...
\noindent It is only when the work is used, whether it is art that is
...
\noindent It is with adaptive maintenance in mind that Naur says in
his conclusion:
\begin{quote}
...
\noindent Earlier in the text, he gives an example of what he means by
this:
\begin{quote}
...
disciplines. It requires that software behave more like a living
thing, able to adapt to a changing environment.

Naur is clear that the method approach to software engineering is
relevant to the theory-building view, for instance, providing model
solutions of typical problems and principles of structuring systems
consisting of many parts and complicated interactions. However, he
differs from the methodologist in that the programmer must ultimately
decide by taking into account the actual program problem to be solved.
...
specifications \cite{milstd498}, which consist of proven templates
...
identifies environments and sites of operation; and includes any other
relevant documents. It includes a section titled Section~4:
Justification for and Nature of Changes. This even includes a
...
of the issues that Naur now raises. They would have, if it were
possible at the time, implemented what he is describing. He
essentially asserts that in order for someone to do adaptive
maintenance on a code base, they first need a programmer who built the
...
relevant aspects and activities of the real world, and of the limit
...
AI now has the ability to sit in a code base from day one. What we are
suggesting is that we go from an operating concept \emph{document} to
what one might call an operating concept \emph{agent}: a professor of
the theory of programming for that particular code base.

The lead engineer on the code base, the person who may not have
written much code in the code base but was responsible for conceptual
integrity, has proficiency in describing how the program should
respond. They can provide a kind of onboarding that, in the past, was
simply impossible.

Further, they have at their disposal things that were not available in
the past. Previously, we had the code itself and written
documentation. Now, an agent can also review the Git history. It can
review all of the pull requests, all the issues that were raised; it
can analyze how the system has actually responded to each nuance of
situation based on the observed behavior.
